...
def check_up_left(x, y):
	if x > 2 and y > 2:
		return grid[y][x]*grid[y-1][x-1]*grid[y-2][x-1]*grid[y-3][x-1]
	else:
		return 0
...
for i in range(20):
	for j in range(20):
		tmp = 0
		tmp = check_up(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_up_right(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_right(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_bottom_right(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_bottom(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_bottom_left(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_left(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
		tmp = check_up_left(i, j)
		high_prod = tmp if tmp > high_prod else high_prod
		
print("Highest product: ", high_prod)